...
def index():
    ...
    results_list = [parse_resfile(f, want_query=False)[:15] for f in res_files]
    return render_template('simple_index.html', results=zip(res_files, results_list))
...
def flaskrun(app, default_host='0.0.0.0', 
    ...
    parser.add_argument('--simple', metavar='simple_output', 
                      help='simplemode: single page, 10 outputs for every query.')
